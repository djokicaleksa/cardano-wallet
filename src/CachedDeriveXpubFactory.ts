...
import { isShelleyPath, indexIsHardened} from './helpers/addresses';
...
function CachedDeriveXpubFactory(derivationScheme, shouldExportPubKeyBulk, deriveXpubsHardenedFn) {
  ...
  async function deriveXpub(absDerivationPath): Promise<Buffer> {
   ...
   */
  async function _deriveXpubsHardenedFn(derivationPaths): Promise<any> {
