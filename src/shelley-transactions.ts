...
function cborizeTxWitnessesShelley(
  ...
  const txWitnessesShelley = shelleyWitnesses.map(
...
function cborizeTxWitnessesByron(byronWitnesses) {
  const txWitnessesByron = byronWitnesses.map(
...
const cborizeTxVotingRegistration = ({
  ...
  nonce,
}): [number, Map<number, Buffer | any>] => [
  61284,
  new Map<number, Buffer | any>([
...
const cborizeTxAuxiliaryVotingData = (
  ...
  new Map<number, Map<number, Buffer | any>>([
